/* EasyCASE V6.5 17/06/2011 11:58:51 */
/* EasyCASE O
If=vertical
LevelNumbers=no
LineNumbers=no
...
* Pedometer.c
*
* Usage: This file will be used to get the Total Steps, Steps(Delta, Last activity, Current Activity),
* User Activity type, Cadence(Steps/sec), Distance( Delta, Last Activity, Currrent Activity), Speed, 
* Dyanamic Step Length, etc.
* 
*                
*
* Author: Rajender.Singh@in.bosch.com & Aibin.Paul@in.bosch.com 
**************************************************************************************************/
/* EasyCASE ( 5
   Disclaimer */
/*************************************************************************************************/
...
/* EasyCASE ( 100012
   File Name For Doxy */
/*! \file Pedometer.c
    \brief  This file will be used to get the Total Steps, Steps(Delta, Last activity, Current Activity),
            User Activity type, Cadence(Steps/sec), Distance( Delta, Last Activity, Currrent Activity), Speed, 
            Dyanamic Step Length etc. */
/* EasyCASE ) */
/* EasyCASE ( 247
...
/* EasyCASE - */
#define C_CHIPID_BMA250E_U8R                     0xF9   /*Chip ID*/
...
U8 V_CHIPIDCHECK_U8R = 0;                                        /* Chip ID check*/
...
extern void gpsapp_trace(char * fmt,...);
/* EasyCASE ) */
/* EasyCASE ) */
/* EasyCASE ( 70
...
unsigned long PEDO_GetStepCount(void)
   {
  // gpsapp_trace("******gpsapp_trace***** = %d",V_StepCount_U16R);
 ...
 *
 ******************************************************************************/
void PEDO_InitAlgo(unsigned char v_GRange_u8r)//初始化算法
   {
   //unsigned char v_addr_u8r=0;
   //unsigned char v_ChipID_u8r=0;
   /* EasyCASE - */
   /*Read Chip id from address 00*/
   
   //bma255_read_reg(v_addr_u8r, &v_ChipID_u8r,1);
   
   //if (/*Check whether chip id is of BMA250(0xFB)*/
    //   v_ChipID_u8r == C_CHIPID_BMA250E_U8R)
      if(1)
      {
      ...
      /*Chip id validation variable made 0.Thus algo will run*/
      V_CHIPIDCHECK_U8R=1;
      }
   else
      {
      /*Chip id is wrong.SO no need to allow algorithm to run.
      Making V_StatusFlags_U8R=0 will make algorithm not to run*/
      V_StatusFlags_U8R = C_Clear_U8X;
      /*Chip id validation variable made 1.Thus algo will not run*/
      V_CHIPIDCHECK_U8R=0;
      }
   }
/* EasyCASE ) */
 ...
 *
 ******************************************************************************/
void PEDO_EnableRobustness(void)//鲁棒性使能
   {
 ...
 *
 ******************************************************************************/
short PEDO_ProcessAccelarationData(short v_RawAccelX_s16r, short v_RawAccelY_s16r, short v_RawAccelZ_s16r)//过程加速数据
   {
   /* EasyCASE ( 101532
   ...
   /* EasyCASE ( 101534
      Reset Previous data */
    //  gpsapp_trace("V_StatusFlags_U8R = %d v_PreStatusFlags_u8r = %d",V_StatusFlags_U8R,v_PreStatusFlags_u8r);
  ...
  // gpsapp_trace("--------1234V_StatusFlags_U8R = %d ",V_StatusFlags_U8R);
   //add by xiekai
   //V_StatusFlags_U8R = 34;
   if (V_StatusFlags_U8R & M_NegativeTrend_U8X)
      ...
      if (V_StatusFlags_U8R & M_PositiveTrend_U8X)
         ...
         /* EasyCASE ) */
		// gpsapp_trace("--------V_StatusFlags_U8R = %d ",V_StatusFlags_U8R);
         if (/*Check whether step is valid or not*/
            ...
            if (V_StatusFlags_U8R & M_Qualified_U8X)
               ...
               if (V_StatusFlags_U8R&M_ModeCounting_U8X)
                  ...
                  /* EasyCASE ( 101554
                     Counting Mode */
                   //  gpsapp_trace("00000000000v_CorrectionCount_u8r = %d ",v_CorrectionCount_u8r);
                  ...
                  /* EasyCASE ( 101555
                     Detection Mode */
                  //   gpsapp_trace("1111111111111111 V_StatusFlags_U8R = %d ",V_StatusFlags_U8R);
 ...
 ******************************************************************************/
 ...
 U32 PEDO_MedianFilter( U32 *p_InputArray_u32r, U8 v_SizeInputArray_u8r)//中值滤波器
   {
   /* EasyCASE ( 101526
      Local Variables */
   /* EasyCASE < */
...
void PEDO_Reset_SingleAxisAccelFeatures(ts_SingleAxis *p_AccelerationPatternFeatures_xxr)//单轴加速度特性
   {
   /* EasyCASE ( 101670
      Local Variable */
   U8 v_Index_u8r=0;
